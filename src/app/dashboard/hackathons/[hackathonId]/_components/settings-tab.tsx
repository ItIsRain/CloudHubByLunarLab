...
export function SettingsTab({ hackathon, hackathonId }: SettingsTabProps) {
  ...
  const [visibility, setVisibility] = React.useState<Visibility>(
    (hackathon.visibility as Visibility) || "public"
  );
  ...
  // Invitation hooks
  const { data: invitations = [] } = useEntityInvitations("hackathon", hackathon.id, visibility === "private");
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Invite Participants (only for private hackathons) */}
      {visibility === "private" && (
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
